...
import {
  PrivateKey as PrivateKeyPkcs8,
  PrivateKeyInfo as PrivateKeyInfoPkcs8,
...
export async function verifyCertificatePem(
...
export async function verifyCertificate(
  ...
  if (date) {
    ...
    throw new Error(
      "Bad value for date. Must be a Date object, false or null.",
    );
  }
  ...
  for (let cert of certs) {
    if (!parentKey)
      throw new Error(
        "Invalid chain, at least one signing certificate does not have the CA flag",
      );
    const result = await verifyCertficateKeys(cert, parentKey, {
      date: validationDate,
      signatureOnly: false,
    });
    if (!result) {
 ...
 */
async function verifyCertficateKeys(
  cert: X509Certificate,
  parentKey: PublicKey,
  params: X509CertificateVerifyParams,
): Promise<boolean> {
  if (!parentKey)
    throw new Error("Missing parentKey key to validate the certificate");
  ...
  if (params.signatureOnly) {
    ...
    return (
      ok && cert.notBefore.getTime() < time && time < cert.notAfter.getTime()
    );
  }
}
 ...
 * @returns
 */
async function verifyCertificateLibsodium(
  tbs: Uint8Array,
  ...
  const rawKey = new Uint8Array(
    parentKey.rawData.slice(parentKey.rawData.byteLength - 32),
  );
...
export class CertificateWrapper {
  ...
  getPublicKey(): string {
    ...
    const publicKeySlice = publicKey.rawData.slice(
      publicKey.rawData.byteLength - 32,
    );
  ...
  getMillegrillePublicKey(): string {
    if (!this.millegrille)
      throw new Error("The millegrille certificate was not provided");
 ...
 */
function extractMillegrillesExtensions(
  certificate: X509Certificate,
): MilleGrillesCertificateExtensions {
  const exchanges = readExtensionListValue(
    certificate.getExtension(OID_EXCHANGES),
  );
  ...
  const adminGrants = readExtensionListValue(
    certificate.getExtension(OID_ADMIN_GRANTS),
  );
  const domainGrants = readExtensionListValue(
    certificate.getExtension(OID_DOMAIN_GRANTS),
  );
...
export function wrapperFromPems(
  pems: string[],
  ca?: string,
): CertificateWrapper {
...
export async function getIdmg(ca: X509Certificate | string) {
  ...
  const certDigest = await digest(new Uint8Array(ca.rawData), {
    encoding: "bytes",
    digestName: "blake2s-256",
  });
  if (!ArrayBuffer.isView(certDigest)) throw new Error("wrong encoding");

  // Encode with multihash
  let certMhDigest = getMultihashBytes(
    "blake2s-256",
    new Uint8Array(certDigest),
  );
...
export function loadPrivateKeyEd25519(
  pem: string,
  password?: string,
): Uint8Array {
  ...
  let privateKey = ecParams.privateKey.slice(
    ecParams.privateKey.byteLength - 32,
  );
  return new Uint8Array(privateKey);
}

export function savePrivateKeyEd25519(
  key: Uint8Array,
  password?: string,
): string {
  if (key.length != 32)
    throw new Error("The private key must by 32 bytes in length");
...
export class CertificateCache {
  maxSize: number;
  cacheContent: {
    [pubkey: string]: { wrapper: CertificateWrapper; date: Date };
  };
  ...
  async saveCertificate(
    chain: string[],
  ): Promise<CertificateWrapper | boolean> {
...
export async function generateCsr(
  ...
  if (userId) {
    let userIdExtension = new Extension(
      OID_USERID,
      false,
